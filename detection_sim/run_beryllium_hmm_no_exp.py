...
if __name__ == "__main__":
    ...
    path = np.repeat('./outputs/beryllium_test_hmm_no_exp_threshold/dark_R_D_0/', len(n_bins))
    ...
    R_dark = np.repeat(0.0, len(n_bins))
    path = np.repeat('./outputs/beryllium_test_hmm_no_exp_threshold/bright_R_D_0/', len(n_bins))
    with mp.Pool(mp.cpu_count()) as p:
        res = p.starmap(run_environment, zip(n_bins, initial_state, R_dark, path))
    ...
    path = np.repeat('./outputs/beryllium_test_hmm_no_exp_threshold/dark_R_D_0_5/', len(n_bins))
    with mp.Pool(mp.cpu_count()) as p:
        res = p.starmap(run_environment, zip(n_bins, initial_state, R_dark, path))
    ...
    initial_state = np.repeat(0, len(n_bins))
    R_dark = np.repeat(0.5/200, len(n_bins))
    path = np.repeat('./outputs/beryllium_test_hmm_no_exp_threshold/bright_R_D_0_5/', len(n_bins)) 
    with mp.Pool(mp.cpu_count()) as p:
        res = p.starmap(run_environment, zip(n_bins, initial_state, R_dark, path))
